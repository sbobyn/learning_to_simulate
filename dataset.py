...
def preprocess(particle_type, position_seq, target_position, metadata, noise_std):
    ...
    edge_displacement = torch.gather(
        ...
        recent_position, dim=0, index=edge_index[1].unsqueeze(-1).expand(-1, dim)
    )
...
class OneStepDataset(pyg.data.Dataset):
    def __init__(self, data_path, split, window_length=7, noise_std=0.0):
        ...
        self.particle_type = np.memmap(
            os.path.join(data_path, f"{split}_particle_type.dat"),
            dtype=np.int64,
            mode="r",
        )
        ...
        if os.path.exists(os.path.join(data_path, f"{split}_step_context.dat")):
            self.step_context = np.memmap(
                os.path.join(data_path, f"{split}_step_context.dat"),
                dtype=np.float32,
                mode="r",
            )
        else:
        ...
        for traj in self.offset.values():
            ...
            for i in range(length):
                ...
                if self.step_context:
                    desc["global"] = (
                        traj["step_context"]["offset"] + i * self.global_feature_dim
                    )
    ...
    def get(self, idx):
        ...
        if self.step_context:
            step_context = self.step_context[
                ...
                + self.window_length * self.global_feature_dim
            ].copy()
...
class RolloutDataset(pyg.data.Dataset):
    ...
    def get(self, idx):
        ...
        position = self.position[
            traj["position"]["offset"] : traj["position"]["offset"]
            + time_step * size * self.dim
        ].copy()
